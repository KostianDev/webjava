...
class GlobalExceptionHandlerTest {
  ...
  void handleNotFoundPropagatesHeaderTraceId() {
    ...
    request.setRequestURI("/api/v2/products/" + UUID.randomUUID());
    ...
    ResponseEntity<ProblemDetail> response = handler.handleNotFound(
      new ProductNotFoundException(UUID.randomUUID()),
      webRequest
    );
  ...
  void handleAllFallsBackToRequestIdHeader() {
    ...
    ResponseEntity<ProblemDetail> response = handler.handleAll(
  ...
  void handleValidationReturnsProblemDetailWithViolations() throws Exception {
    ...
    bindingResult.addError(
      new FieldError("productDTO", "name", "must not be blank")
    );
    bindingResult.addError(
      new FieldError("productDTO", "price", "must be positive")
    );

    Method method = Objects.requireNonNull(
      DummyController.class.getDeclaredMethod("create", ProductDTO.class)
    );
    ...
    ProblemDetail body = (ProblemDetail) Objects.requireNonNull(
      response.getBody()
    );
    var props = Objects.requireNonNull(body.getProperties());
  ...
  void handleFeatureNotAvailableReturns501WithFeatureName() {
    MockHttpServletRequest request = new MockHttpServletRequest();
    request.setRequestURI("/api/v2/cosmo-cats");
